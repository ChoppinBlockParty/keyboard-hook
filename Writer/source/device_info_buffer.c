...
#define KEYBOARD_HOOK_WRITER_DEVICE_INFO_BUFFER_DEVICE_NAME \
  "keyboard_hook_writer_device_info_buffer"
...
int open_device_info_buffer(struct inode* inode, struct file* filp) {
  ...
  if (mj != device_info_buffer->major ||
      mn != device_info_buffer->minor) {
...
ssize_t write_to_device_info_buffer(struct file*       filp,
  ...
  if ((device_info_buffer->bufferPosition + count) >=
      device_info_buffer->buffer_size) {
    count = device_info_buffer->buffer_size - device_info_buffer->bufferPosition;
  }

  if (copy_from_user(&(device_info_buffer->data[
                         device_info_buffer->bufferPosition]),
                     buf, count) != 0) {
    retval = -EFAULT;
    goto out;
  }
...
int create_device_info_buffer(unsigned int  major,
  ...
  device_info_buffer = (struct device_info_buffer*)kzalloc(
    sizeof(struct device_info_buffer),
    GFP_KERNEL);
  ...
  if (device_info_buffer->data == NULL) {
    device_info_buffer->data =
      (unsigned char*)kzalloc(device_info_buffer->buffer_size, GFP_KERNEL);
...
void destroy_device_info_buffer(void) {
  ...
  device_destroy(device_info_buffer->class,
                 MKDEV(device_info_buffer->major, device_info_buffer->minor));
  cdev_del(&device_info_buffer->cdev);
